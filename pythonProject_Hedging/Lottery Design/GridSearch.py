...
def generate_HR(scenario):
    ...
    for i in range(rounds):
        ...
        # Grid search for LPM
        h_values = np.linspace(-100, 100, 10001)  # 10001 points from -100 to 100
        lpm_values = [lpmObjective(h, p, x_h, x_l, y_h, y_l) for h in h_values]
        best_h_index = np.argmin(lpm_values)
        results['hr_lpm'][i] = h_values[best_h_index]
