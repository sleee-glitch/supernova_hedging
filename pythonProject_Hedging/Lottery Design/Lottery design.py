...
def generate_HR(scenario):
    ...
    for i in range(rounds):
        ...
        result_mv = minimize(varianceObjective, 0, args=(p, x_h, x_l, y_h, y_l), method='BFGS')
        ...
        # Multiple starting points for LPM optimization
        starting_points = [-10, -1, 0, 1, 10, results['hr_mv'][i]]  # Wider range of starting points
        lpm_results = []

        for start in starting_points:
            result_lpm = minimize(lpmObjective, start, args=(p, x_h, x_l, y_h, y_l),
                                  method='BFGS',
                                  options={'maxiter': 1000, 'gtol': 1e-8},
                                  bounds=[(-100, 100)])  # Add bounds to search a wider range
            if isinstance(result_lpm, OptimizeResult) and result_lpm.success:
                lpm_results.append((result_lpm.fun, result_lpm.x[0]))

        if lpm_results:
            best_lpm = min(lpm_results, key=lambda x: x[0])
            results['hr_lpm'][i] = best_lpm[1]
        else:
            results['hr_lpm'][i] = np.nan  # If no successful optimization, set to NaN

        # Print debugging information
        print(f"Iteration {i}:")
        print(f"p: {p}, x_h: {x_h}, x_l: {x_l}, y_h: {y_h}, y_l: {y_l}")
        print(f"LPM results: {lpm_results}")
        print(f"Best LPM: {results['hr_lpm'][i]}")
        print("---")


        # Choose the result with the lowest objective function value
        best_lpm = min(lpm_results, key=lambda x: x[0])
        results['hr_lpm'][i] = best_lpm[1]
